  ...
  return (
    ...
    >
      ...
      <section className="py-12 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 text-center max-w-4xl mx-auto relative z-10">
        <GlassBadge variant="primary" className="mb-4 font-semibold">Deep Dives</GlassBadge>
        <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-slate-900 dark:text-white tracking-tight font-display mb-6">
          ...
          <span className="bg-gradient-to-r from-brand-primary via-brand-secondary to-brand-accent bg-clip-text text-transparent">Quantifiable Results</span>
        </h1>
      ...
      {/* --- CASE STUDIES LIST --- */}
      <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16 sm:space-y-20 relative z-10">
        {studies.map((study, idx) => {
          ...
          return (
            <motion.div
              ...
              className="flex"
            >
              <GlassCard className="hover:border-brand-primary/30 w-full p-8 sm:p-12 text-left relative overflow-hidden" hoverEffect="lift">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-start">
                  
                  {/* Stats & Metadata Column */}
                  <div className={`lg:col-span-4 space-y-6 ${isEven ? 'lg:order-first' : 'lg:order-last'}`}>
                    <GlassBadge variant="primary" className="font-semibold">
                      {study.category}
                    </GlassBadge>
                    
                    <div>
                      <span className="text-[10px] uppercase text-slate-400 dark:text-slate-500 font-bold tracking-widest block">Client Partner</span>
                      <h3 className="text-xl sm:text-2xl font-extrabold text-slate-900 dark:text-white font-display mt-1.5">{study.client}</h3>
                    </div>

                    <div className="grid grid-cols-1 gap-4 pt-5 border-t border-slate-200/50 dark:border-white/5">
                      {study.stats.map((stat, i) => (
                        <div key={i} className="flex justify-between items-end border-b border-slate-200/30 dark:border-white/5 pb-2.5">
                          <span className="text-xs text-slate-500 dark:text-slate-400 font-semibold">{stat.label}</span>
                          <span className="text-base sm:text-lg font-bold text-brand-primary">{stat.value}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  <div className="lg:col-span-8 space-y-6">
                    ...
                    <p className="text-sm sm:text-base text-slate-500 dark:text-slate-350 leading-relaxed italic border-l-2 border-brand-primary/40 pl-4 py-1">
                      "{study.summary}"
                    </p>
                    ...
                    <div className="space-y-6 pt-6 border-t border-slate-200/50 dark:border-white/5">
                      ...
                      <div className="flex gap-4">
                        ...
                        <div>
                          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200 mb-1">The Challenge</h4>
                      ...
                      <div className="flex gap-4">
                        <div className="w-9 h-9 rounded-xl bg-brand-primary/10 border border-brand-primary/20 flex items-center justify-center shrink-0">
                          <Code2 size={15} className="text-brand-primary" />
                        </div>
                        <div>
                          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200 mb-1">Our Strategy & Execution</h4>
                      ...
                      <div className="flex gap-4">
                        ...
                        <div>
                          <h4 className="text-xs font-bold uppercase tracking-wider text-slate-800 dark:text-slate-200 mb-1">Business Impact</h4>
                          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 leading-relaxed">{study.result}</p>
                        </div>
                      </div>
